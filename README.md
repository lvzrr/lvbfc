...
| `&`      | Pointer Writer        | Writes the address of `buf + (N - 1)` into the tape, from the current cell (as a raw `uintptr_t`) to the current cell + N. Allows constructing absolute syscall arguments. Stackable: e.g. `&&&` writes address of `buf + 2` at `buf`, does no bounds checking. |
...
- [ ] Intrinsics and builtins system
